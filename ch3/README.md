...
Authorization requires the grant to exist **and** the caller's verified token to carry the scope —
two independent controls, so a policy mistake alone is not sufficient to authorize an action.
...
Real Ed25519 signing via `node:crypto` — no dependencies, and the signature tests are genuine
